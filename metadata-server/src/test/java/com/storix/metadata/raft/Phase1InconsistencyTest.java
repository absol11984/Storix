...
/**
 * Phase 1 Inconsistency Test
 *
 * Tests that deliberately inconsistent committed generations are detected and fail safely.
 * The system must NOT silently combine files from different generations.
 */
class Phase1InconsistencyTest {
    ...
    // ===== TEST 1: INCONSISTENT GENERATION =====

    /**
     * INCONSISTENCY TEST
     *
     * Create a deliberately inconsistent state:
     *   commit marker = generation 11
     *   snapshot = generation 11
     *   metadata = generation 10 (mismatched!)
     *
     * Expected: explicit recovery failure
     *
     * The system must NOT silently merge generations.
     */
    @Test
    void testInconsistentGenerationDetection() throws Exception {
        System.out.println("\n========================================");
    ...
    // ===== TEST 2: CORRUPTED SNAPSHOT WITH COMMIT MARKER =====

    /**
     * Test that a corrupted snapshot with valid commit marker fails safely.
     */
    @Test
    void testCorruptedSnapshotWithCommitMarkerFailsSafely() throws Exception {
        System.out.println("\n========================================");
        System.out.println("TEST: Corrupted Snapshot With Commit Marker");
        System.out.println("========================================\n");

        Path snapDir = tempDir.resolve("snapshots");
        Path metaFile = tempDir.resolve("metadata-corrupt.json");
        Files.createDirectories(snapDir);

        // Create valid metadata
        MetadataStore store = new MetadataStore(metaFile);
        ObjectMetadata obj = makeObject("test", 1000L);
        store.createObjectDirect(obj);
        store.setGeneration(5);
        store.save();

        // Create valid snapshot
        SnapshotManager sm = new SnapshotManager(snapDir, store);
        sm.takeSnapshot(5, 1);
        sm.commitGeneration(5, 1);

        // Corrupt the snapshot
        Path snapshotFile = snapDir.resolve("snapshot-5");
        byte[] content = Files.readAllBytes(snapshotFile);
        content[content.length - 10] ^= 0xFF; // Corrupt some bytes
        Files.write(snapshotFile, content);

        System.out.println("Corrupted snapshot-5");

        // Restart - should detect corruption
        System.out.println("Restarting with corrupted snapshot...");
        ...
        // Should not return a valid snapshot
        assertTrue(latestSnap.isEmpty(),
            "Corrupted snapshot should not be loaded");

        System.out.println("Corrupted snapshot correctly rejected");

        System.out.println("\n========================================");
        System.out.println("TEST: Corrupted Snapshot - PASSED");
        System.out.println("========================================\n");
    }
}
